...
@router.get("/{release_id}/changes", response_model=ReleaseChanges)
def release_changes(release_id: int, conn: psycopg.Connection = Depends(get_conn)):
    """The code this release ships: the umbrella chart diffed between the
    previous release's tag and this one's, each changed service diffed between
    its version tags, commits mapped to tickets. Computed live — a hosting that
    cannot be asked surfaces as an error, never as "no changes"."""
    rel = _load(conn, release_id)
    cfg = appconfig.effective(conn)
    try:
        ...
        raise HTTPException(503, f"The git hosting cannot be queried: {exc}") from exc
    except GitUnreachable as exc:
        raise HTTPException(502, f"Git hosting query failed: {exc}") from exc
    return git_changes.changes_view(cs)


@router.post("/{release_id}/code-review", response_model=DocumentMeta, status_code=201)
def run_code_review(
    release_id: int,
    conn: psycopg.Connection = Depends(get_conn),
    principal: Principal = Depends(current_principal),
):
    """Run the advisory AI code review over this release's changes. The report
    lands as a DRAFT "Code Review Report" document (a new version on re-runs).
    Synchronous — expect roughly one LLM call per changed component."""
    rel = _load(conn, release_id)
    cfg = appconfig.effective(conn)
    try:
        result = code_review.run_code_review(conn, cfg, principal, rel)
