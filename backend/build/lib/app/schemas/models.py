...
class GitRepoLinkCreate(BaseModel):
    ...
    umbrella Chart.yaml, which change detection joins on."""
    provider: Literal["github", "gitlab"]
    repo: str = Field(min_length=1, description='GitHub "owner/repo" or GitLab "group/project"')
    role: Literal["component", "library", "deployment"]
    component_name: str = ""
...
class GitRepoLinkUpdate(BaseModel):
    ...
    repo: str | None = None
    role: Literal["component", "library", "deployment"] | None = None
...
class ReleaseChanges(BaseModel):
    """The code changes of a release, computed live: umbrella Chart.yaml diffed
    between the previous release's tag and this one's, then each changed
    service diffed between its two version tags. Nothing is cached. Raw file
    diffs never leave the backend — this is the commit/version view."""
    release_id: int
    version: str
