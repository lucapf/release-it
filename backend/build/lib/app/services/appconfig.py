...
# Git hosting connections (source repositories). The repositories themselves
# are bound per-product (product_git_repository), like the tracker project.
GIT_GITHUB_ENABLED = "git_github_enabled"
GIT_GITHUB_BASE_URL = "git_github_base_url"
GIT_GITHUB_TOKEN = "git_github_token"
GIT_GITLAB_ENABLED = "git_gitlab_enabled"
...
SECRET_KEYS = {JIRA_TOKEN, GITHUB_TOKEN, CLAUDE_API_KEY, GIT_GITHUB_TOKEN, GIT_GITLAB_TOKEN}
...
class EffectiveConfig:
    ...
    llm: LLMConfig
    # Git hosting connections — both may be enabled at once (unlike trackers):
    # one product's repos can span GitHub and a self-hosted GitLab.
    git_github: GitProviderConfig = field(
        default_factory=lambda: GitProviderConfig(False, "", "")
    )
    git_gitlab: GitProviderConfig = field(
        default_factory=lambda: GitProviderConfig(False, "", "")
    )
...
def effective(conn: psycopg.Connection) -> EffectiveConfig:
    ...
    return EffectiveConfig(
        ...
        git_github=GitProviderConfig(
            enabled=b(GIT_GITHUB_ENABLED, settings.git_github_enabled),
            base_url=s(GIT_GITHUB_BASE_URL, settings.git_github_base_url),
            token=s(GIT_GITHUB_TOKEN, settings.git_github_token),
        ),
        git_gitlab=GitProviderConfig(
            enabled=b(GIT_GITLAB_ENABLED, settings.git_gitlab_enabled),
            base_url=s(GIT_GITLAB_BASE_URL, settings.git_gitlab_base_url),
            token=s(GIT_GITLAB_TOKEN, settings.git_gitlab_token),
        ),
    )
