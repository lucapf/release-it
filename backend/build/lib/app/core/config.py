...
class Settings(BaseSettings):
    ...
    # --- Git hosting (source repositories) ----------------------------------
    # Connections to the git APIs the linked product repositories live on.
    # Independent of the issue-tracker GitHub connection (a product may track
    # tickets in Jira while its code is on GitHub) and of the GitLab CI pipeline
    # trigger settings above (whose base_url is a trigger endpoint, not an API
    # root). Seed defaults; the configuration page (app_config) overrides them.
    git_github_enabled: bool = False
    git_github_base_url: str = "https://api.github.com"
    git_github_token: str = ""

    git_gitlab_enabled: bool = False
    git_gitlab_base_url: str = ""  # instance root, e.g. https://gitlab.example.com
    git_gitlab_token: str = ""
