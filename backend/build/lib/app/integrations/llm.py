...
class LLMCompletion(Protocol):
    """A single-shot completion: one system prompt, one user prompt, one text
    answer. For service-layer jobs (e.g. the code review) that must not run
    through the chat agent loop and its tool budget."""
...
class ClaudeProvider:
    ...
    def draft_release_notes(self, version: str, issues: list[dict]) -> str:
        return self.complete(_SYSTEM, _prompt(version, issues), max_tokens=2048)
...
class OllamaProvider:
    ...
    def draft_release_notes(self, version: str, issues: list[dict]) -> str:
        return self.complete(_SYSTEM, _prompt(version, issues))
...
def get_completion_service(cfg: LLMConfig) -> LLMCompletion:
    """The configured engine as a generic single-shot completion service.
    Same resolution (and same no-stub stance) as release notes."""
    return get_release_note_service(cfg)  # both providers implement complete()
