...
def _fill_web_url(conn: psycopg.Connection, row: dict) -> dict:
    """Fill an empty web_url with the provider's standard repository page, so
    the stored field is only an override and the UI always has a link to show
    (when one can be derived at all)."""
    cfg = appconfig.effective(conn)
    row["web_url"] = git.repo_web_url(cfg, row["provider"], row["repo"], row["web_url"])
    return row


def _normalize_web_url(
    conn: psycopg.Connection, provider: str, repo_path: str, web_url: str
) -> str:
    """Store the URL only when it differs from the derived default. Responses
    fill the derived URL in, so an edit round-trips it back here — persisting
    it would freeze a value that should keep tracking the provider connection."""
    cfg = appconfig.effective(conn)
    if web_url and web_url == git.repo_web_url(cfg, provider, repo_path):
        return ""
    return web_url
...
def _friendly_link_conflict(exc: pg_errors.UniqueViolation) -> HTTPException:
    ...
    if "one_anchor" in name:
        return HTTPException(
            409,
            "This product already has a version-anchor repository (an umbrella "
            "chart or a codebase repo) — it can only have one.",
        )
...
def list_git_repos(product_id: int, conn: psycopg.Connection = Depends(get_conn)):
    ...
    return [
        ...
        for r in git_repos_repo.list_for_product(conn, product_id)
    ]
...
def add_git_repo(
    ...
    try:
        return _fill_web_url(conn, git_repos_repo.create(
            conn,
            ...
            web_url=_normalize_web_url(conn, body.provider, repo_path, body.web_url.strip()),
...
def update_git_repo(
    ...
    try:
        return _fill_web_url(conn, git_repos_repo.update(
            conn,
            link_id,
            provider=provider,
            repo=repo_path,
            ...
            web_url=(_normalize_web_url(conn, provider, repo_path, body.web_url.strip())
                     if body.web_url is not None else None),
